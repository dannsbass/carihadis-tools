<?php
/**
 * script untuk men-generate file database dari mysql
 */
if(file_exists('log'))unlink('log');
$awal = microtime(true);
$koneksi = mysqli_connect('localhost','root','','testdata');
$query = "SELECT * FROM database_hadis";
...
while($data = $mysql_query->fetch_assoc()){
    $nass = str_replace("\n"," ",$data['nass']);
    $terjemah = str_replace("\n"," ",$data['terjemah']);
    $hno = $data['hno'];
    $nomor_kitab = (int)trim($data['nama_kitab']);
    if($nomor_kitab > 16) break;
    $nama_kitab = $kitabs[$nomor_kitab - 1];
    file_put_contents('database_hadis',"$nass | $terjemah | $nama_kitab | $hno\n",FILE_APPEND|LOCK_EX);
    $log = "$nama_kitab:$hno ";
